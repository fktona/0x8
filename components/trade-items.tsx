...
import { cn, formatNumber, formatNumber2, timeAgo } from "@/libs/utils";
...
function TradeItems({
  ...
  const tokenValue = (token: string, amount: string) => {
    ...
    const value = parseFloat(amount) / parseFloat(token);
    return value;
  };
  ...
  return (
    <div className="flex w-full flex-wrap text-[10px] justify-start 2xl:justify-between  items-center gap-[6px] text-white/80 font-light">
      ...
      <span className="text-white/80 font-light w-10 text-ellipsis truncate ">
        {item.name}
      </span>
      <span
        className={
      ...
      >
        {item.type === "buy"
          ? `${formatNumber(item.tokenOutAmount)} ${
              item.tokenOutSymbol.length > 4
                ? item.tokenOutSymbol.slice(0, 4) + "..."
                : item.tokenOutSymbol
            }`
          : `${formatNumber(item.tokenOutAmount)}  ${
              item.tokenInSymbol.length > 4
                ? item.tokenInSymbol.slice(0, 4) + "..."
                : item.tokenInSymbol
            }`}
      </span>
